...
export default class Inventory {
    ...
    update() {
        this.allToolTips.forEach((item) => {
            ...
            if(this.orientationY) {
                item.item.x = targetX - (item.item.getBounds().width + hPadding) * -1.1;
                item.item.y = this.scene.tooltip.getTarget(item.id).getBounds().y;
            } else {
                
                item.item.x = targetX - (item.item.getBounds().width + hPadding) * 0.5;
                item.item.y = this.scene.tooltip.getTarget(item.id).getBounds().y - 76;
